...
(define-struct/contract d:number ([value number?]) #:transparent)
(define-struct/contract d:variable ([name symbol?]) #:transparent)
(define-struct/contract d:lambda ([params (listof d:variable?)] [body d:term?]) #:transparent)
(define-struct/contract d:closure ([env handle?] [decl d:lambda?]) #:transparent)
(define-struct/contract d:bool ([value boolean?]) #:transparent)
(define-struct/contract d:builtin ([func (-> d:value? d:value?)]) #:transparent)
(define-struct/contract d:apply ([func d:expression?] [args (listof d:expression?)]) #:transparent)
(define-struct/contract d:define ([var d:variable?] [body d:expression?]) #:transparent)
(define-struct/contract d:seq ([fst d:term?] [snd d:term?]) #:transparent)
