...
class JNegateOp extends JUnaryExpression {
    ...
    public JExpression analyze(Context context) {
        ...
        if (operand.type == Type.INT) {
            ...
            type = Type.INT;
        } else if (operand.type == Type.LONG) { // project5/problem2 implementing else if for long to analyze the type long
            operand.type().mustMatchExpected(line(), Type.LONG);
            type = Type.LONG;
        } else if (operand.type == Type.DOUBLE) { // project5/problem2 implementing else if for double to analyze the type double
...
class JPostIncrementOp extends JUnaryExpression {
     ...
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) { // project5/problem2 implemented the analyze for int for postincrement
        operand = operand.analyze(context);
        operand.type().mustMatchExpected(line(), Type.INT);
        type = Type.INT;
        return this;
    ...
    public void codegen(CLEmitter output) {
        if (operand instanceof JVariable) {
            // A local variable; otherwise analyze() would have replaced it with an explicit
            // field selection.
            // project5/problem2 implemented the codegen for int for postincrement
            int offset = ((LocalVariableDefn) ((JVariable) operand).iDefn()).offset();
            if (!isStatementExpression) {
                // Loading its original rvalue.
                operand.codegen(output);
...
class JPreDecrementOp extends JUnaryExpression {
     ...
     * {@inheritDoc}
     */
    public JExpression analyze(Context context) {  // project5/problem2 implemented the analyze for int for preincrement
        operand = operand.analyze(context);
        operand.type().mustMatchExpected(line(), Type.INT);
        type = Type.INT;
        return this;
    ...
    public void codegen(CLEmitter output) {
        if (operand instanceof JVariable) {
            // A local variable; otherwise analyze() would have replaced it with an explicit
            // field selection.
            // project5/problem2 implemented the codegen for int for predecrement
            int offset = ((LocalVariableDefn) ((JVariable) operand).iDefn()).offset();
            output.addIINCInstruction(offset, -1);
            if (!isStatementExpression) {
                // Loading its original rvalue.
